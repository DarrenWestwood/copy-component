...
function copyToClipboard(evt) {
  ...
  navigator.clipboard.writeText(copy_html).then(() => {
    processOverlay(copy_element);
  });
}

var processOverlay = function (copy_element) {
  var copy_input = copy_element.querySelector(".output-copy");
  ...
  setTimeout(function () {
    // Hide copied overlay
    hideOverlay(copied_overlay, copy_value);
  }, 3000);
};
...
function createCopiedOverlay(copy_value, copy_element) {
  ...
  copied_overlay = addOverlayStyles(copied_overlay, boxStyles, copy_value);
  return copied_overlay;
}

function addExistingStyles(boxStyles) {
...
function addOverlayStyles(copied_overlay, boxStyles, copy_value) {
...
function showOverlay(copied_overlay, copy_value) {
  copied_overlay.style.display = "inline-block";
  copy_value.style.display = "none";
}

function hideOverlay(copied_overlay, copy_value) {
...
function getCheckImage(iconColor) {
  if (iconColor == "light")
...
.output-copy-container .blockonomics-icon[onclick]:hover {
